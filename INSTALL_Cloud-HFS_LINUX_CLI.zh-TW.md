...
本指南將引導您如何在 Linux 伺服器、NAS 或無頭（Headless）環境中下載、部署與使用 **SUMA Cloud-HFS Linux CLI 版本 (cloud-hfs-daemon)**。
...
下載完成後，請將執行檔移動到您想要運行的目錄，並賦予執行權限：
...
```
*啟動後會自動在同目錄下產生 `cloud-hfs-daemon.pid` 記錄進程編號，且會預設自動開啟檔案分享服務。*
...
- **背景服務日誌（Log）**：會存放在 `~/.config/cloud-hfs/cloud-hfs-daemon.log`
...
1. 使用瀏覽器登入 Web 管理介面。
2. 拍選 **「啟動遠端分享」**（首次啟用系統會自動安裝 `cloudflared` 元件）。
3. 通道建立成功後，即可複製公開的分享網址，或點選取得專屬 `suma.tw` 短網址分享給遠端親友。
