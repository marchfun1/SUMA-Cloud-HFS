...
本指南将引导您如何在 Linux 服务器、NAS 或无头（Headless）环境中下载、部署与使用 **SUMA Cloud-HFS Linux CLI 版本 (Cloud-HFS)**。
...
下载完成后，请先将压缩包解压至家目录下的 `suma-cloud-hfs` 目录，并赋予执行权限：

```bash
# 解压 tar.gz 至 ~/suma-cloud-hfs (以 0.10.5 版本为例)
...
```
*启动后会自动在同目录下生成 `Cloud-HFS.pid` 记录进程编号，且会默认自动开启文件分享服务。*
...
- **后台服务日志（Log）**：会存放在 `~/.config/cloud-hfs/Cloud-HFS.log`
